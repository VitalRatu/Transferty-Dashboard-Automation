import { Page} from '@playwright/test';
import { FilterBar } from '../../../related_components/FilterBar'; 
import { Table } from '../../../related_components/Table';
import { Pagination } from '../../../related_components/Pagination';
import { BasePage } from '../../../BasePage';

export type ChannelProviderListPageFilters = 
    | 'Project' 
    | 'Provider' 

export class ChannelProviderListPage extends BasePage
{
    public readonly filterBar: FilterBar<ChannelProviderListPageFilters>;
    
    public readonly table: Table;

    public readonly pagination: Pagination;

    constructor(page: Page, tabDepth = 0) 
    {
        super(page, /\/billing\/balances\/channel\/provider/);
        this.filterBar = new FilterBar<ChannelProviderListPageFilters>(page);
        this.table = new Table(page);
        this.pagination = new Pagination(page);
    }
}